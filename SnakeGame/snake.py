...
class cube(object):
    ...
    def __init__(self, start, dirnx=1, dirny=0, color=(251, 212, 14)):
        self.pos = start
        self.dirnx = 1
        self.dirny = 0
        self.color = color
...
def save_highscore(highscore):
    with open('DB/highscore.txt', 'w') as file:
        file.write(str(highscore))

def load_highscore():
    if os.path.isfile('DB/highscore.txt'):
        with open('DB/highscore.txt', 'r') as file:
...
def main():
    ...
    key_pressed = False  # Track if an arrow key is pressed

    clock = pygame.time.Clock()
    current_score = 0

    pygame.mixer.music.load("Music/AfterLose.mp3")
    ...
    while flag:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:  # Check for key press
                    if event.key in (pygame.K_LEFT, pygame.K_RIGHT, pygame.K_UP, pygame.K_DOWN):
                        ...
                        sound_effect = pygame.mixer.Sound("SoundEffects/start.wav")  # Replace with your sound effect file path
                        sound_effect.play()
                        pygame.mixer.music.load("Music/SnakeMusic1.mp3")
                        pygame.mixer.music.play(-1)
                        sound_effect = pygame.mixer.Sound("SoundEffects/Run.wav")  # Replace with your sound effect file path
                        sound_effect.play(-1)
        ...
        if game_started:
            ...
            if s.body[0].pos == snack.pos:  # jei gyvate suvalgo snacks (uzeina ant snack pozicijos, gyvate pailgeja vienu bloku)
                ...
                snack = cube(randomSnack(rows, s), color=(179, 25, 25))
                current_score += 1
                sound_effect = pygame.mixer.Sound("SoundEffects/Eat.wav")  # Replace with your sound effect file path
                sound_effect.play()

            for x in range(len(s.body)):
                if s.body[x].pos in list(map(lambda z: z.pos, s.body[x + 1:])):
                    sound_effect = pygame.mixer.Sound("SoundEffects/Punch.wav")  # Replace with your sound effect file path
                    sound_effect.play()
                    if current_score > cube.highscore:
                        cube.highscore = current_score
                        save_highscore(cube.highscore)
                        sound_effect = pygame.mixer.Sound("SoundEffects/Success.wav")  # Replace with your sound effect file path
                        sound_effect.play()
                        pygame.mixer.music.load("Music/AfterLose.mp3")
                        pygame.mixer.music.play(-1)
                        message_box('New Highscore', "Press OK To Play Again!")
                    if current_score < cube.highscore:
                        sound_effect = pygame.mixer.Sound("SoundEffects/Lose.wav")  # Replace with your sound effect file path
                        sound_effect.play()
                        pygame.mixer.music.load("Music/AfterLose.mp3")
                        pygame.mixer.music.play(-1)
